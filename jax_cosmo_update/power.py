...
def get_pkz_mult_mat(matrix_params, k, a, k_matrix, z_matrix):
    z_query = 1.0 / a - 1.0
    delta_k = np.log(k_matrix)[1] - np.log(k_matrix)[0]
    Delta_k = np.log(k) - np.log(k_matrix[0])
    indk = Delta_k/delta_k
    delta_z = z_matrix[1] - z_matrix[0]
    Delta_z = z_query - z_matrix[0]
    indz = Delta_z/delta_z
    mat_val = jax.scipy.ndimage.map_coordinates(matrix_params, [indk, indz], order=1, mode='nearest')
    return mat_val



def nonlinear_matter_power(
    cosmo, k, a=1.0, matrix_params=np.ones((5,5)), k_matrix=np.logspace(-2,1,5), z_matrix=np.linspace(0,1,5), transfer_fn=tklib.Eisenstein_Hu, nonlinear_fn=halofit
):
    """Computes the non-linear matter power spectrum.

    This function is just a wrapper over several nonlinear power spectra.
    """
    mat_val = get_pkz_mult_mat(matrix_params, k, a, k_matrix, z_matrix)
    return mat_val * nonlinear_fn(cosmo, k, a, transfer_fn=transfer_fn)
