...
sys.path.append(os.path.dirname(path_here + '../../'))
import jax_cosmo_update as jc
import jax_cosmo_update.background as bkgrd
import jax_cosmo_update.constants as const
import jax_cosmo_update.power as power
import jax_cosmo_update.transfer as tklib
from jax_cosmo_update.scipy.integrate import simps
from jax_cosmo_update.utils import a2z
from jax_cosmo_update.utils import z2a
...
def angular_cl(
    cosmo, ell, probes, matrix_params=np.ones((5,5)), k_matrix=np.logspace(-2,1,5), z_matrix=np.linspace(0,1,5), transfer_fn=tklib.Eisenstein_Hu, nonlinear_fn=power.halofit
):
    """
    ...
    def cl(ell):
        def integrand(a):
            ...
            pk = power.nonlinear_matter_power(cosmo, k, a, matrix_params, k_matrix, z_matrix, transfer_fn, nonlinear_fn)
